...
<a style="color: black; font-family:sans-serif,impact;position: absolute;left: 260px;top: 350px;" href="/club/list">All clubs</a>
<a style="color: black; font-family:sans-serif,impact;position: absolute;left: 260px;top: 380px;" href="/club/add">Add club</a>
<form class="form" action="/club/list" method="POST">
    <div class="field">
        <label for="num">Id club</label>
        <input class="input" type="number" name="num" value="0">
        <br>
        <input style="left: 193px;top: 20px;" class="button" type="submit" value="Ok">
    </div>
</form>

<form class="form2" action="/club/list" method="GET">
    <label style="font-family: sans-serif,impact;font-size: 20px;
    font-weight: bold;" for="num2" >Count sportsmen: ${queryForm.num2}</label>
</form>
...
<div class="table_dark">
    <table>
        <tr>
            ...
            <th>Name</th>
            <th>Sport</th>
            <th>firstPlaceClubId</th>
            <th>secondPlaceClubId</th>
            <th>thirdPlaceClubId</th>
            <th>Delete</th>
            <th>Edit</th>
        </tr>
        <#list clubFormList as tempClub>
            <tr>
                <td>${tempClub.id}</td>
                <td>${tempClub.name}</td>
                <td>${tempClub.sport}</td>
                <td>${tempClub.countFirstPlace}</td>
                <td>${tempClub.countSecondPlace}</td>
                <td>${tempClub.countThirdPlace}</td>
                <td><a href="/club/delete/${tempClub.id}" style="color: #47e7db;text-decoration: none">Delete</a></td>
                <td><a href="/club/edit/${tempClub.id}" style="color: #47e7db;text-decoration: none">Edit</a></td>
            </tr>
        </#list>
    </table>
</div>
